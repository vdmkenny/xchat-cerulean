...
#include <openssl/x509v3.h>	  /* X509_CHECK_FLAG_() */
...
static void
ASN1_TIME_snprintf (char *buf, int buf_len, const ASN1_TIME * tm)
{
	char *expires = NULL;
...
_SSL_get_cert_info (struct cert_info *cert_info, SSL * ssl)
{
	...
	/* X509 is opaque since OpenSSL 1.1, so ask for the fields by accessor. */
	{
		...
		X509_PUBKEY_get0_param (&algo_obj, NULL, NULL, NULL,
										X509_get_X509_PUBKEY (peer_cert));
		alg = OBJ_obj2nid (algo_obj);
	}
	sign_alg = X509_get_signature_nid (peer_cert);
	ASN1_TIME_snprintf (notBefore, sizeof (notBefore),
							  X509_get0_notBefore (peer_cert));
	ASN1_TIME_snprintf (notAfter, sizeof (notAfter),
							  X509_get0_notAfter (peer_cert));
...
SSL *
_SSL_socket (SSL_CTX *ctx, int sd, const char *hostname)
{
	SSL *ssl;
