...
- (NSMenu *)menuForURL:(NSString *)url inSession:(struct session *)sess
{
    ...
    NSMenuItem *imageMenu = [[[NSMenuItem alloc] initWithTitle:NSLocalizedStringFromTable(@"Open with internal viewer", @"xchataqua", @"") action:@selector(openWithInternalViewer:) keyEquivalent:@""] autorelease];
    [imageMenu setTarget:self];
    [imageMenu setToolTip:url]; // just a trick to avoid writing data source
    [menu addItem:imageMenu];

    NSMenuItem *copyMenu = [[[NSMenuItem alloc] initWithTitle:NSLocalizedStringFromTable(@"Copy URL", @"xchataqua", @"") action:@selector(copyURL:) keyEquivalent:@""] autorelease];
    [copyMenu setTarget:self];
    [copyMenu setToolTip:url]; // just a trick to avoid writing data source
...
- (NSString *)stripImageFromTitle:(NSString *)title icon:(NSString **)icon
{
    ...
    [mTitle replaceOccurrencesOfString:@"_" withString:@"" options:NSCaseInsensitiveSearch range:NSMakeRange(0, [mTitle length])];
    title = [NSString stringWithString:mTitle];
    [mTitle release];
    ...
    if ([[title substringFromIndex:length-1] isEqualToString:@"~"]) {
        ...
        if (icon) {
            *icon = [title substringWithRange:NSMakeRange(r.location+1, length-r.location-2)];
        }
